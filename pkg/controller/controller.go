...
func (c *ExtendedResourceController) syncClaim() {
	workFunc := func() bool {
		...
		if podName, ok := extendedResourceClaim.Annotations[PodNameAnnotation]; ok {
			...
			if errors.IsNotFound(err) || pod == nil {
				...
				extendedResourceClaim.Status.Message = ""
				extendedResourceClaim.Status.Reason = ""
				extendedResourceClaim.Spec.ExtendedResourceNames = make([]string, 0)
				delete(extendedResourceClaim.Annotations, PodNameAnnotation)
